"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.existUserId = exports.existEmail = void 0;
const user_model_1 = require("../models/user.model");
const existEmail = (email = '') => __awaiter(void 0, void 0, void 0, function* () {
    const emailExist = yield user_model_1.User.findOne({ where: { email } });
    if (emailExist) {
        throw new Error("El correo " + email + " ya esta registrado");
    }
});
exports.existEmail = existEmail;
const existUserId = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const IdExist = yield user_model_1.User.findByPk(id);
    if (!IdExist) {
        throw new Error("El id " + id + " no fue encontrado");
    }
});
exports.existUserId = existUserId;
//# sourceMappingURL=users-validators.js.map